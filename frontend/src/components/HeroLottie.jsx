  ...
  return (
    <section className="relative bg-black text-white overflow-hidden">
      <video className="absolute inset-0 w-full h-full object-cover opacity-70 will-change-transform" autoPlay loop muted playsInline
        style={{ transform: `translateY(${offsetY * 0.5}px) scale(${1 + offsetY * 0.0005})` }}>
        <source src="/hero-animation.mp4" type="video/mp4" />
        Your browser does not support the video tag.
      </video>
      <div className="absolute inset-0 bg-black opacity-30"></div>
      <div className="relative container mx-auto px-6 py-20 md:py-32 lg:py-40 flex flex-col items-start max-w-4xl" style={{ transform: `translateY(${offsetY * 0.2}px)`, opacity: 1 - offsetY * 0.001 }}>
